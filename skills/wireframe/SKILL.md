...
**Slideshow is not prototype.** Slideshow pages through frames in section order — a presentation. Prototype mode navigates via hotspots wired to flow logic — a simulation. Slideshow is always available; prototype mode exists only when the user asked for a prototype. Both ride the same single HTML artifact.
...
- **Prototype hotspots** — accent-outlined regions indicating clickable areas, with a "show hotspots" toggle in the chrome.
...
>
> - **HTML** (default) — one self-contained file: grid/slideshow viewer, optional click-through prototype
...
.board-title { font-family: var(--mono); font-size: 12px; font-weight: 600;
...
.view-toggle button {
  ...
  padding: var(--s1) var(--s3); background: none;
...
body[data-view="slides"] .slide-nav {
  ...
  margin-top: var(--s5); font-family: var(--mono); font-size: 10px;
  letter-spacing: 0.06em; text-transform: uppercase; color: var(--chrome-ink);
}
...
.wf .btn { display: inline-block; padding: var(--s2) var(--s4); border-radius: 3px;
...
.note-rail { max-width: 280px; margin-top: var(--s4); font-size: 12px; color: var(--w-ink2); }
.note-rail .note-num { color: var(--note); font-family: var(--mono); font-weight: 700; }
.hotspot { position: absolute; border: 2px dashed var(--note); border-radius: 3px;
  cursor: pointer; opacity: 0; transition: opacity 120ms; background: transparent; }
body[data-hotspots="on"] .hotspot { opacity: 0.85; }
```
    ...
    <div class="frame-grid">
      <article class="frame" id="scr-[slug]" style="width: [VIEWPORT]px">
        ...
        <div class="wf" style="position: relative;">
          ...
          <!-- prototype hotspots (only when requested): -->
          <button class="hotspot" data-go="scr-[target-slug]"
                  style="top: …; left: …; width: …; height: …;"
                  aria-label="Go to [target]"></button>
        </div>
      </article>
      <!-- thumbnails use: <article class="frame thumb" style="width:160px"> with .zone divs -->
    </div>
...
hs?.addEventListener('click', () => {
  ...
  document.body.dataset.hotspots = on ? 'on' : 'off';
  hs.setAttribute('aria-pressed', String(on));
});
document.querySelectorAll('.hotspot').forEach(h =>
  h.addEventListener('click', () => {
    const t = document.getElementById(h.dataset.go);
    if (!t) return;
    if (document.body.dataset.view === 'slides') show(frames.indexOf(t));
...
Materialize flow logic from `/journey` as connected screens. Lay frames in flow order, draw connections in the container layer, and label what triggers each transition. When the user wants a prototype, wire hotspots over the actual trigger elements — the real button, the real list row — not whole-screen jumps, so walking the prototype rehearses the real interaction. Prototypes are click-through only: no conditional logic, no state simulation. When a flow needs state to be testable, that's a finding to hand back to `/journey`, not a feature to fake.

### 7. Divergence practice

For any new screen, thumbnails come first and come plural. Draw structurally different options — not the same layout with the sidebar moved. Different answers to "what is this screen's primary job?" produce different structures; if all your thumbnails look alike, you've decided the job without noticing. Present alternatives with the trade-off named, converge deliberately with the user, then promote the chosen structure up a rung. Keep the rejected thumbnails in the artifact under a "considered" section — the record of what was chosen against is part of the design.
...
5. **Considered & rejected** — Thumbnails that lost, with one line each on why.
