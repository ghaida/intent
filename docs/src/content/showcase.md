...
Nearby is a hyperlocal marketplace for buying, selling, and trading within walking distance. It is not an e-commerce platform — it's a neighborhood infrastructure tool. The business model is density-dependent: value accrues as exchange volume increases per square mile, not per user. Revenue comes from optional promoted listings and neighborhood business partnerships, never from transaction fees. The product must reach critical mass within a single neighborhood before expanding geographically — growth is concentric, not scattered.

**Users**
- **Sellers:** People with stuff they want gone — moving, decluttering, upgrading. Not professional resellers. They want minimal effort: snap a photo, name a price, done. Key context: most have tried Craigslist or Facebook Marketplace and found it too heavy for a $10 lamp. They post in bursts (moving weekend, garage cleanout), not steadily. Tolerance for friction is near zero — if posting takes more than 60 seconds, they'll just leave it on the curb with a "FREE" sign.
- **Buyers:** Bargain hunters and convenience seekers. They'd rather walk two blocks than wait two days for shipping. They browse casually and buy impulsively. Key context: discovery is more compelling than search — they don't know what they want until they see it. The walking-distance constraint is a feature, not a limitation. They treat the app like window shopping, not like targeted purchasing.
- **Organizers:** Block captains, community garden coordinators, buy-nothing group admins. They facilitate exchanges that aren't strictly transactional — lending, gifting, sharing. Key context: they are currently managing this work through Facebook Groups, group texts, or paper flyers. Their pain isn't "I need a marketplace" — it's "I need better tools for what I'm already doing." They are the most likely early evangelists and the hardest to disappoint.
...
**Ethical Stance**
- No engagement tricks. No "3 others are looking at this" urgency. No notification spam. If users open the app less often because they found what they needed quickly, that is success.
...
**Success Definition**
Success is not user count or GMV. Success is exchange density: the number of completed exchanges per 1,000 households per week within a walkable radius. A neighborhood where 20 things change hands every week — some sold, some traded, some given away — is a successful neighborhood. A city with 100,000 users spread across 50 neighborhoods with no density anywhere is a failure.
...
**Gap**
Existing platforms treat geography as a filter, not a feature. You search Facebook Marketplace and filter by "within 5 miles" — but the experience is identical whether the item is 2 blocks away or 20 miles away. No platform builds trust, discovery, or coordination around walkability. The result: exchange completion rates on broad platforms hover around 20-30% for local items, because logistics (driving across town for a $15 item) and trust (meeting a stranger from the internet) remain unresolved.

**Opportunity**
If proximity is the organizing principle — not a filter on top of a nationwide marketplace — the exchange experience fundamentally changes. Logistics collapse to "walk over and pick it up." Trust shifts from algorithmic reputation to neighborhood familiarity. Discovery becomes spatial ("what's near me right now?") instead of keyword-based. This isn't a better Craigslist. It's a new category: neighborhood infrastructure for physical exchange.
...
4. Build neighborhoods where 25%+ of listings are free — generosity is a feature, not an edge case
...
4. Trust is ambient, not scored — neighborhood tenure and proximity replace star ratings
...
- *Assumption:* People will trust neighbors more than strangers from the internet. *Risk:* Proximity can increase safety concerns, not just reduce them — especially for vulnerable populations.
...
- **Geographic split:** 2 neighborhoods — one urban (density >5,000 hh/km²), one suburban (density 1,000-3,000 hh/km²). This tests whether "walking distance" means fundamentally different things in different contexts.
...
- No ratings. No star scores. Trust is ambient, not transactional.
...
- Interest notifications batched every 15 minutes to prevent alert fatigue (not real-time)
...
| Trust Engine unavailable | Profiles show "Nearby member" without tenure/exchange details | Low — trust signals degraded but not absent | Graceful degradation; trust data populated on recovery |
...
| 3. Undo window | Toast overlay | "Interest sent. Undo" — 5 second countdown | If tapped: reverts to pre-interest state. If expired: interest confirmed server-side. |
...
- **Free-text unlocks** after 3 successful exchanges between the same two users. Trust is earned, not assumed.
- **Auto-delete:** Messages delete 48 hours after exchange completes. This isn't a social network — no conversation history to mine.
...
- **Why this pattern:** Nearby's tasks are distinct and self-contained (browse, post, coordinate, manage). Hub-and-spoke keeps each mode clean. The spatial twist: the "hub" is a map, not a traditional home screen — the user's physical location is the organizing principle.
...
| Free Stuff | Anything priced at $0 | — | **Elevated to top-level.** Not buried as a price filter. |
...
- Distance grouping in list view: "On your block" / "5-min walk" / "10-min walk" — gives spatial structure to a flat list.
...
| Warmth | Clinical ←→ Warm | 70% toward warm | "Nice — that's your 8th exchange" not "Transaction #8 complete" |
| Specificity | Generic ←→ Local | 90% toward local | "New in Capitol Hill" not "New near you" |
| Humor | Serious ←→ Playful | 40% toward playful | Light touches, never forced. "It was probably great" not "LOL sorry!" |
| Brevity | Detailed ←→ Terse | 75% toward terse | These are sidewalk transactions. Copy matches the stakes. |

**Voice Principles**
1. **Neighborly, not corporate.** The app is a bulletin board, not a platform. It speaks like a friendly neighbor, not a brand.
2. **Casual, not sloppy.** Friendly tone with clear information. Contractions are fine. Emoji are not — warmth comes from words, not symbols.
3. **Local, not generic.** Use neighborhood names whenever available. "New in Capitol Hill" not "New near you." The specificity builds belonging.
...
- Screen 3: "We need your location to show what's nearby." [Allow location] / [Not now → explain consequence: "Without location, Nearby can't show you anything. You can enable it later in Settings."]
...
- **Exchange history:** "12 exchanges in Capitol Hill" — activity without judgment. Count only, no rating.
...
- **New user:** "New to Nearby" — honest badge for accounts under 4 weeks. Better than hiding newness — transparency builds trust faster than faking tenure.
...
| Time reference | "[Relative time], not [absolute time]" | "Posted 2 hours ago" not "Posted at 3:47 PM" |
...
| H3 | User control and freedom | 2 | No undo on interest expression. No ability to retract after the 5-second toast expires. No "hide this item" for things you've seen and don't want. No way to unsee a category (persistent filter instead). | Interest flow, browse experience |
...
| H9 | Help users recover from errors | 2 | Error messages are well-written but inconsistently applied. Photo upload failure has good recovery. Network loss during posting has good draft preservation. But: expired listing error has no path to re-post. Failed interest has no retry. Server errors show generic fallback. | Error states across flows |
...
| E-08 | P2 | No undo on interest after toast | After the 5-second undo window, interest cannot be retracted. Changed your mind? Too bad. Need either extended undo or explicit "withdraw interest." | /journey |
...
Every state the item card can be in, across the full lifecycle. Not just "default" and "error" — every messy in-between state that real usage produces.
...
| Loading | Skeleton placeholder (gray shapes matching card layout) | Nothing — wait | Auto-resolves in <2s; timeout at 5s → error state |
...
| Seller ghosts after accepting interest | Item auto-relists after 2 hours; buyer notified; seller nudged | **Pass** | 3 ghost events in 30 days → "Slow mode" on seller account |
...
| 100 simultaneous interest expressions on one item | First interest served; others queued; server batches notifications | **Pass** | Race condition handled by optimistic locking; "Someone got there first" for losers |
...
| Listing sits for 14 days with no interest | Day 12: "Still available?" push notification. No response → auto-archive. | **Pass** | One-tap re-post available from archive |
...
- *Design:* Post-exchange feedback is optional and binary: "As described" or "Not as described." No public reviews. No star ratings. 3 "Not as described" flags in 30 days surface a private prompt to seller: "A few neighbors felt items didn't match descriptions. Adding detailed photos helps build trust."
- *No public shaming.* This is a neighborhood, not eBay. Escalation is private and constructive.
...
- *Design:* One account per phone number. Neighborhood assignment based on consistent geolocation over 7 days, not self-reported address. Multiple accounts from same device flagged for review.
...
The map-first interface is fundamentally inaccessible as a primary experience for screen reader users. This is not a bug to patch — it's an architectural decision to revisit. The list view must be promoted from "toggle alternative" to "equal citizen." When VoiceOver or TalkBack is active, the app should default to list view, not map view.
...
**Critical requirement:** The undo toast ("Interest sent. Undo") must be announced via aria-live and the undo button must be keyboard-focusable for its full 5-second duration. A timed action that screen reader users can't reach is not an undo — it's an inaccessible pattern.
...
- Instructions use active voice and short sentences: "Tap the photo to retake it." Not "The photo can be retaken by tapping on it."
...
- No "3 others are interested" urgency messaging. Ever. This is a design principle, not a feature flag.
...
| Gesture alternatives | Every swipe gesture has a tap alternative | Swipe-to-interest → "I want this" button. Pinch-to-zoom radius → tap presets (5/10/20 min). |
...
| Reduced motion | `prefers-reduced-motion` respected | Spring animations → instant transitions. Map panning → jump-to-location. Skeleton shimmer → static placeholder. |
...
- **Spatial cognition:** Distance shown in both time and distance: "4 min walk (0.2 mi)" — for users who process spatial information differently. On map view, distance is also represented visually by the radius circle.
- **Low vision and map readability:** Map pins use high-contrast colors (not just hue — luminance differentiation). Price labels on pins use minimum 14px text. "Free" items use a distinct pin shape (not just color) to be distinguishable without color perception.
...
Mobile is not "the app shrunk down." Mobile is the product. Nearby is designed for someone walking through their neighborhood, phone in hand, glancing at what's around them. The interaction model is built for this context:
...
The web experience is a complement, not a clone. Feature parity is explicitly not a goal. The web version exists for three use cases that mobile handles poorly:
...
Nearby must work on sidewalks, in basements, and on subway platforms. "No signal" is not an error state — it's a Tuesday.
...
**Cross-device anti-pattern avoided:** No "continue where you left off" interstitials. The state just syncs. If you open the laptop and the app shows your neighborhood, you don't need a modal telling you that it synced. Seamless is the point.
...
Trust is not universal. What builds trust between strangers in Portland, Oregon destroys it in Osaka, Japan. The entire trust model must be locally adapted — not translated, reconceived.
...
| **Japan** | Platform mediation (the platform vouches, not the individual) | Account verification level, membership duration, formality of communication | Direct contact before platform introduction. Casual first-message tone. | Add honorific-aware messaging. Platform sends the first introduction message, not the buyer. Seller "accepts meeting request" — not "responds to interest." |
...
| **Germany** | Data minimalism + explicit consent | Account age, verified email, GDPR-compliant data handling badge | Showing proximity without consent ("3 min walk" feels surveillant) | Distance shown only after explicit opt-in. Privacy controls on first screen of onboarding, not buried in settings. Default radius display: "In your neighborhood" (no distance). |
...
Full layout mirroring for Arabic, Hebrew, and Urdu. Not just text direction — the entire spatial model flips:
...
- German: 30-40% longer than English. "I want this" → "Interesse bekunden" (18 chars → 20 chars — manageable). But "Your neighbor since March 2025" → "Ihr Nachbar seit März 2025" + "Dein Nachbar seit..." (formal vs. informal address decision required per market).
...
| Photo norms | Item-centric (faces incidental) | In some markets, showing faces is avoided. Make item-only photos the norm by defaulting to item-focused camera framing. No face detection prompts. |
...
| Currency display | $15, $0 = "Free" | Localized currency symbol + format. "Free" translated per market. "¥0" is not "Free" in Japan — use the word. |
...
| **Digital Markets Act** | EU | If Nearby reaches gatekeeper status (unlikely near-term), interoperability requirements. | Monitor. Not actionable at current scale. |
...
| Message-to-exchange ratio | Average messages exchanged per completed transaction | <3 | Fewer messages = better coordination design. More messages = friction |
| Neighborhood coverage | % of city neighborhoods with 10+ active users | Grow 5%/month | Growth must be geographic, not just aggregate. 10 users per neighborhood is minimum viable density |
...
| Organizer engagement | % of neighborhoods with at least one active organizer | 50% | Organizers are the immune system — they moderate, they seed, they set norms |
...
|---|---|---|
| **Time in app** | More time = UX is failing, not succeeding. If someone browses for 20 minutes, discovery is broken. | Optimizing time-in-app leads to infinite scroll, algorithmic feeds, and engagement traps. The opposite of what Nearby should be. |
...
| **DAU/MAU ratio** | High DAU/MAU means people feel compelled to check daily. That's addiction, not utility. | We'd add streaks, daily rewards, FOMO triggers. The opposite of respecting user time. |
...
|---|---|---|---|---|---|
| Walking radius default: 10 min vs 15 min | Wider default radius increases exchange density in low-density neighborhoods without diluting trust | Exchange density, browse-to-interest rate, "as described" rate | A/B test by neighborhood (not by user — everyone in a neighborhood sees the same radius) | 4 weeks | 20 neighborhoods per arm |
...
**Experiment governance:** No experiment ships without a pre-registered hypothesis, primary metric, counter-metric, and minimum sample size. Results are reviewed by product and design together — not just "did the number go up?" but "did it go up for the right reasons?"
...
| CTA | "I want this" — full-width button, 48px height, primary color background, white text, 8px border-radius. Positioned in bottom safe area (persistent, not scrolled away). |
...
2. Apply barrier masking: remove all area beyond physical barriers, even if within straight-line radius. Walking radius follows the street graph, not a circle.
...
**Fallback logic:**
- If street network graph unavailable: use straight-line distance at 1.3x multiplier (empirical correction for non-straight walking paths). Circle instead of isochrone.
