...
3. **Annotation (accent):** numbered markers, note rails, and vignette caption indexes in a single accent color (Intent indigo by default, user-overridable — the override never touches the kit's content accent). Annotation shares the indigo but speaks in its own shapes — markers and rail text, never controls. Notes are per wireframe, never one consolidated list: markers number per frame, the rail groups each frame's notes under its name, and the slideshow shows only the active frame's group. Prototype interactivity is not an overlay — it lives on the wireframe's own elements.
